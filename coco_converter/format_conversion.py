...
class my_converter:
    ...
    def converter(self):
        ...
        # filename = annotations["file_name"]
            ...
            if last_img_id != annotations["image_id"]:#if new image
                for image in data['images']:
                    if image['id'] == annotations["image_id"]:
                        img_width = image['width']
                        img_hight = image['height']
                        break
                if anno_count == 0:
                    ...
                    if self.save_txt_flag:
                        self.save_txt(annotations,img_width,img_hight)
        ...
        if self.display_flag:
            self.annocheck(data)
        print("Done!\n")
        print("The converted json file is saved in: ",os.path.join(self.ana_txt_save_path,self.save_type+"_converted"".json"))
    ...
    def save_txt(self, data, img_width, img_hight):
        bbox = data['bbox']
        img_id = data['image_id']
        keypoints = data['keypoints']
        bbox[:2] = self.val_norm(bbox[:2], img_width, img_hight, self.accuracy)
        bbox[2:] = self.val_norm(bbox[2:], img_width, img_hight, self.accuracy)

        keypoints = [self.val_norm(keypoints[i:i+2], img_width, img_hight, self.accuracy) + [2] for i in range(0, len(keypoints), 3)]
